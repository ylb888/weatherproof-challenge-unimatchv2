...
The `pretrained/` directory is ignored by git because model weights are large.
...
sh scripts/test_weatherproof.sh \
  ...
  exp/weatherproof/unimatch_v2/dinov2_base_train_all/best_ema.pth \
  data/test_input \
...
Compare prediction folders against a reference prediction set:

```bash
python scripts/compare_prediction_miou.py \
  --reference output/weatherproof/test_predictions \
  --candidates output/weatherproof/test_predictions
```

The script reports mIoU in percentage format. For example, `mIoU=47.3202` means `47.3202%`.
...
Avoid committing datasets, checkpoints, generated predictions, and cache files.
